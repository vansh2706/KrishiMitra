...
import { Upload, Camera, X, Loader2, Bug, AlertTriangle, CheckCircle, Download, Share2 } from 'lucide-react'
...
export function EnhancedPestDetection({ voiceEnabled, onSpeak }: EnhancedPestDetectionProps) {
    ...
    // Language change detection and re-analysis - removed to prevent page reload
    // Instead, we'll handle language changes within the same result display
        ...
        const pestsData = {
            ...
            'Bollworm': {
                ...
                ta: {
                    ...
                    chemicalTreatment: ['குளோரான்டிரானிலிப்ரோல்', 'ஸ்பினோசாட்', 'இமாமெக்டின் பென்சோயேட்'],
    ...
    const analyzePest = async (imageUrl: string, lang: string = language) => {
        ...
        try {
            console.log('Analyzing pest with language:', lang)
            
            // Real AI analysis using Gemini (with language support)
            const analysisResult = await geminiImageAnalysis(imageUrl, lang)
...
This report is for informational purposes only. Please consult with agricultural experts for professional advice.`
            ...
            a.href = url
            a.download = `pest-analysis-report-${result.pestName.replace(/[^a-zA-Z0-9]/g, '_')}-${Date.now()}.txt`
