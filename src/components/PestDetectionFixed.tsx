...
export function PestDetectionFixed({ voiceEnabled, onSpeak }: PestDetectionFixedProps) {
    ...
    const analyzePest = async (imageUrl: string) => {
        ...
        try {
            ...
            // Mock result
            const mockResult: PestDetectionResult = {
                pestName: 'Aphids',
                confidence: 87,
                severity: 'medium',
                description: 'Small insects that feed on plant sap',
                symptoms: ['Yellowing leaves', 'Stunted growth', 'Honeydew deposits'],
                treatment: ['Apply neem oil', 'Use insecticidal soap', 'Remove affected parts'],
                prevention: ['Regular monitoring', 'Encourage beneficial insects', 'Proper spacing']
            }

            setResult(mockResult)

            if (voiceEnabled) {
                const announcement = `Pest detected: ${mockResult.pestName} with ${mockResult.confidence}% confidence.`
                                    ...
                                    <CardContent className="space-y-4">
                                        <div className="flex items-center justify-between">
                                            ...
                                            <div className="text-right">
                                                ...
                                                <p className="text-sm mt-1">{result.confidence}% Confidence</p>
                                            </div>
                                        </div>
