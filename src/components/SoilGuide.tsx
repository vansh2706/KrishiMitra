...
export function SoilGuide({ voiceEnabled, onSpeak }: SoilGuideProps) {
    ...
    const handleAnalyze = (lang: string = language) => {
        ...
        const fetchSoilData = async () => {
            try {
                console.log('Fetching soil data with language:', lang)
                await new Promise(resolve => setTimeout(resolve, 500)) // Simulate network delay
                
                // Get language-aware soil data
                const soilData = getSoilDataForLanguage(selectedSoil, lang) || soilDatabase[selectedSoil]
                
                // Apply language-specific processing here if needed
                const processedData = {
                    ...soilData,
                    // Add language-specific content transformation
                    fetchedWithLanguage: lang,
                ...
                if (voiceEnabled && soilData) {
                    const summary = lang === 'hi'
                        ? `${soilData.type} के लिए सुझाव: pH ${soilData.pH}, उर्वरता ${soilData.fertility}`
                        : `Recommendations for ${soilData.type}: pH ${soilData.pH}, fertility ${soilData.fertility}`
                    onSpeak(summary)
                }
            } catch (error) {
                    ...
                    <div className="grid gap-4 md:grid-cols-2">
                        <div>
                            ...
                            <Select value={selectedSoil} onValueChange={setSelectedSoil}>
                                ...
                                <SelectContent>
                                    <SelectItem value="clay">{t('clay')} / चिकनी मिट्टी</SelectItem>
                                    <SelectItem value="sandy">{t('sandy')} / बलुई मिट्टी</SelectItem>
                                    <SelectItem value="loamy">{t('loamy')} / दोमट मिट्टी</SelectItem>
                                    <SelectItem value="black">Black / काली मिट्टी</SelectItem>
                                    <SelectItem value="red">Red / लाल मिट्टी</SelectItem>
                                    <SelectItem value="alluvial">Alluvial / जलोढ़ मिट्टी</SelectItem>
                                    <SelectItem value="laterite">Laterite / लैटेराइट मिट्टी</SelectItem>
                                    <SelectItem value="saline">Saline / लवणीय मिट्टी</SelectItem>
                                    <SelectItem value="alkaline">Alkaline / क्षारीय मिट्टी</SelectItem>
                                    <SelectItem value="acidic">Acidic / अम्लीय मिट्टी</SelectItem>
                                    <SelectItem value="organic">Organic / जैविक मिट्टी</SelectItem>
                                    <SelectItem value="rocky">Rocky / पथरीली मिट्टी</SelectItem>
                                    <SelectItem value="volcanic">Volcanic / ज्वालामुखी मिट्टी</SelectItem>
                                    <SelectItem value="coastal">Coastal / तटीय मिट्टी</SelectItem>
                                    <SelectItem value="mountain">Mountain / पहाड़ी मिट्टी</SelectItem>
                                    <SelectItem value="forest">Forest / वन मिट्टी</SelectItem>
                                </SelectContent>
                            </Select>
                        </div>
