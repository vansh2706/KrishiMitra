...
// Run this from the terminal: node test-chatbot.js

const { deepseekChat } = require('./src/deepseek-api.ts')

async function testChatbot() {
    console.log('Testing DeepSeek chatbot functionality...')
    ...
    try {
        ...
        console.log('✅ DeepSeek Chatbot test successful!')
        console.log('Response:', response.choices[0]?.message?.content)

    } catch (error) {
        console.error('❌ DeepSeek Chatbot test failed:', error.message)
        console.log('\n📝 To fix this issue:')
